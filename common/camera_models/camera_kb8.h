...
class COMMON_API KB8Camera : public PinholeCamera {
 ...
 public:
  ...
  KB8Camera(const vector<Tdata> &distcoef, cv::FileStorage &fs_settings, int id, bool &bmiss_param);
  ...
  void SetvLappingArea(const vector<int> &vlappingarea) {
    vlappingarea_ = vlappingarea;
    if (image_scale_ != (Tdata)(1.f)) {
      for (auto &itmp : vlappingarea_) itmp = (int)(itmp * image_scale_);
    }
  }
