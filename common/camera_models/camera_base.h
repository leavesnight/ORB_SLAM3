...
#include <math.h>
#include <memory>
#include "common/sophus/se3_extra.h"
...
class GeometricCameraBase {  // designed for body(imu/ref/...) frame to cam:Tbc
 public:
  ...
  using CamId = uint8_t;
  using SE3data = Sophus::SE3ex<Tdata>;
...
class CameraBase {
 public:
  ...
  virtual Eigen::Vector2d Project(const Vec3io &p_3d) const {}
  ...
  Tdata Getimage_scale() const { return image_scale_; }

 protected:
  Tsize width_;
  Tsize height_;
  vector<Tdata> parameters_;
  Tdata image_scale_ = 1.f;
