...
class COMMON_API PinholeCamera : public GeometricCamera {
 ...
 public:
  ...
  PinholeCamera(cv::FileStorage &fs_settings, int id, bool &bmiss_param);
  static bool ParseCamParamFile(cv::FileStorage &fs_settings, int id, Camera::Ptr &pCameraInstance);
  ...
  Eigen::Vector2d Project(const Vec3io &p_3d) const override;
...
void PinholeCamera::Project(const Vec3io &p_3d, Vec2data *p_img, Mat23io *d_img_d_p3d, Mat2Xio *d_img_d_param) const {
  const auto x = (Tcalc)p_3d[0];
  const auto y = (Tcalc)p_3d[1];
  const auto z = (Tcalc)p_3d[2];
  ...
  if (d_img_d_p3d) {
    ...
    (*d_img_d_p3d) << fx * invz, 0, -fx * x * invz2, 0, fy * invz, -fy * y * invz2;
  }
