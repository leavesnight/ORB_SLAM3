...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame, i))
                {
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const bool bRight1 = (mpCurrentKeyFrame -> NLeft == -1 || idx1 < mpCurrentKeyFrame -> NLeft) ? false
                                                                                                         : true;
            const float kp1_ur = bRight1 ? mpCurrentKeyFrame->mvuRight[idx1 - mpCurrentKeyFrame->NLeft] : mpCurrentKeyFrame->mvuRight[idx1];
            ...
            const bool bRight2 = (pKF2 -> NLeft == -1 || idx2 < pKF2 -> NLeft) ? false
                                                                               : true;
            const float kp2_ur = bRight2 ? pKF2->mvuRight[idx2 - pKF2->NLeft] : pKF2->mvuRight[idx2];
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
          ...
          auto imubias = (*itKF)->mPrevKF->GetImuBias();
            dirG -= (*itKF)->mPrevKF->GetImuRotation() * (*itKF)->mpImuPreintegrated->GetDeltaVelocity(imubias);//GetUpdatedDeltaVelocity();
